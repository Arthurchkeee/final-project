...
public class CommentDaoImpl implements CommentDao {
    ...
    private static final String SELECT_COMMENTS_BY_BOOK="SELECT* FROM comment WHERE book=?";
    ...
    public List<Comment> findCommentsByBook(Long id){
        List<Comment> comments=new ArrayList<>();
        try(Connection connection=ConnectionPool.getInstance().getConnection();
        PreparedStatement preparedStatement=connection.prepareStatement(SELECT_COMMENTS_BY_BOOK)) {
            preparedStatement.setLong(1,id);
            ResultSet resultSet=preparedStatement.executeQuery();

            while (resultSet.next()){
                comments.add(new Comment(resultSet.getLong("id"),resultSet.getString("username"),resultSet.getLong("book"),resultSet.getDate("date"),resultSet.getString("text")));
            }
